...
const genFrontMatrix = (width: number, height: number, fill: CellType = 'unknown') => {
...
const genBackMatrix = (width: number, height: number, minesCount: number, [excludeY, excludeX]: number[]) => {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            for (const [dy, dx] of aroundOffsets) {
                const ay = y + dy
                const ax = x + dx
                if (ay >= 0 && ay < height && ax >= 0 && ax < width && matrix[ay][ax] === 'mine') {
                    minesAround++
                }
            }
...
const openFieldCell = (state: FieldState, y: number, x: number) => {
    ...
    if (state.backField[y][x] === '0') {
        for (const [dy, dx] of aroundOffsets) {
            const ay = y + dy
            const ax = x + dx
            if (ay >= 0 && ay < height && ax >= 0 && ax < width && ['unknown', 'qm'].includes(state.frontField[ay][ax])) {
...
export const selectIsSmileClicked = (state: RootState) => state.field.isSmileClicked

export const selectFrontField = (state: RootState) => state.field.frontField

export const selectTimerStarted = (state: RootState) => state.field.timerStartedAt > 0

export const selectIsGameOver = (state: RootState) => state.field.isLost || state.field.isWin

export const selectMinesCount = (state: RootState) => state.field.minesCount

export const selectTimerSeconds = (state: RootState) => Math.floor((state.field.currentTime - state.field.timerStartedAt) / 1000)

export const { setSmileType, setIsSmileClicked, onSmileClick, openCell, markCell, updateTimer } = fieldSlice.actions
